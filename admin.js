...
async function renderCasesTab() {
  ...
  try {
    ...
    if (sessions.length === 0) {
      el.innerHTML = '<div class="empty-state"><div class="icon"><img src="assets/icons/clipboard-white.png" class="ic" alt=""></div><div class="text">No Cases working on at the moment</div></div>';
      return;
    }
  ...
  fetch('data/status.json?_=' + Date.now()).then(function(r){ return r.json(); }).then(function(data) {
    ...
    if (s.pending_permissions && s.pending_permissions.length > 0) {
      s.pending_permissions.forEach(function(p) {
        ...
        html += '<img src="assets/icons/shield-white.png" class="ic" alt="" style="margin-bottom:3px">';
