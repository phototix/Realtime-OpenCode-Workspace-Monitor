  ...
  sessions.forEach(ss => {
    ...
    card.innerHTML = `
      ...
      <div style="display:flex;gap:6px;align-items:center;margin-top:6px;padding-top:4px;border-top:1px solid var(--border)">
        ...
        ${(ss.todos || []).filter(function(t){ return t.status !== 'completed'; }).length > 0 ? `<span style="color:var(--yellow);cursor:pointer;font-size:11px" onclick="showTasks('${ss.id}')" title="View tasks">\uD83D\uDCDD ${(ss.todos || []).filter(function(t){ return t.status !== 'completed'; }).length}</span>` : ''}
