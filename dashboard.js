  ...
  sessions.forEach(ss => {
    ...
    card.innerHTML = `
      ...
      <div style="display:flex;gap:6px;align-items:center;margin-top:6px;padding-top:4px;border-top:1px solid var(--border)">
        ...
        ${loggedIn && ss.state !== 'thinking' && ss.state !== 'running-tools' ? `<button class="comment-btn" onclick="event.stopPropagation();continueSession('${ss.id}')" title="Quick instruction"><img src="assets/icons/chat-white.png" class="ic-sm" height="12" alt=""></button>` : ''}
        <button class="sc-copy-id" onclick="event.stopPropagation();navigator.clipboard.writeText('${ss.id}').then(function(){showToast('Session ID copied','success')}).catch(function(){})" title="Copy Session ID" style="background:none;border:none;cursor:pointer;font-size:11px;padding:0 2px;color:var(--text-dim);line-height:1"><img src="assets/icons/clipboard-white.png" class="ic-sm" height="12" alt="Copy"></button>
        ${(ss.todos || []).filter(function(t){ return t.status !== 'completed'; }).length > 0 ? `<span style="color:var(--yellow);cursor:pointer;font-size:11px" onclick="showTasks('${ss.id}')" title="View tasks"><img src="assets/icons/report-white.png" class="ic-sm" height="12" alt=""> ${(ss.todos || []).filter(function(t){ return t.status !== 'completed'; }).length}</span>` : ''}
        ${(ss.pending_permissions || []).length > 0 ? `<span style="color:var(--yellow);cursor:pointer;font-size:11px" onclick="showQuestions('${ss.id}')" title="Permission required"><img src="assets/icons/shield-white.png" class="ic-sm" height="12" alt=""> ${(ss.pending_permissions || []).length}</span>` : ''}
        ${(ss.pending_questions || []).length > 0 ? ((ss.pending_questions || []).filter(function(q){ return !q.answered; }).length > 0 ? `<span style="color:var(--yellow);cursor:pointer;font-size:11px" onclick="showQuestions('${ss.id}')"><img src="assets/icons/support-white.png" class="ic-sm" height="12" alt=""> ${(ss.pending_questions || []).filter(function(q){ return !q.answered; }).length}</span>` : `<span style="color:var(--green);cursor:pointer;font-size:11px" onclick="showQuestions('${ss.id}')"><img src="assets/icons/like-white.png" class="ic-sm" height="12" alt=""></span>`) : ''}
    ...
    activeCases.slice(0, 8).forEach(ss => {
      ...
      div.innerHTML = `
        ...
        <div style="display:flex;gap:4px;margin:2px 0 0 12px;flex-wrap:wrap;font-size:9px">
          ${(ss.pending_permissions || []).length > 0 ? `<span style="color:var(--yellow);cursor:pointer" onclick="showQuestions('${ss.id}')"><img src="assets/icons/shield-white.png" class="ic-sm" height="12" alt=""></span>` : ''}
          ${(ss.pending_questions || []).length > 0 ? ((ss.pending_questions || []).filter(function(q){ return !q.answered; }).length > 0 ? `<span style="color:var(--yellow);cursor:pointer" onclick="showQuestions('${ss.id}')"><img src="assets/icons/support-white.png" class="ic-sm" height="12" alt=""> ${(ss.pending_questions || []).filter(function(q){ return !q.answered; }).length}</span>` : `<span style="color:var(--green);cursor:pointer" onclick="showQuestions('${ss.id}')"><img src="assets/icons/like-white.png" class="ic-sm" height="12" alt=""></span>`) : ''}
        </div>
      `;
      sl.appendChild(div);
    });
